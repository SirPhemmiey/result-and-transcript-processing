...
$status =$_POST['status'];
	...
	$checkSQL = "SELECT matric FROM std WHERE matric = '$matric'";
	$result1 = dbQuery($checkSQL);
	if (mysqli_num_rows($result1) > 0) {
		$_SESSION['msg'] = 'You have added this student already';
...
function addEngg()
{
	...
	$checkSQL = "SELECT email FROM tbl_engineer WHERE ename = '$EngineerName'";
	$result1 = dbQuery($checkSQL);
	if (mysqli_num_rows($result1) > 0) {
		$_SESSION['msg'] = 'You have added this staff already';
...
function addDept()
{
	...
	$checkSQL = "SELECT * FROM department WHERE department.name = '$department'";
	$result1 = dbQuery($checkSQL);
	if (mysqli_num_rows($result1) > 0) {
		$_SESSION['msg'] = 'You have added this department already';
...
function addCourses()
{
	...
	// print_r($_POST). "<br />". print_r($matric); exit();
	for ($i=1; $i<=$loop; $i++) {
		$id = $_POST['course_'.$i];
		// $sql = "INSERT INTO students_courses (id, course_id, matric) VALUES (null, '$id', '$matric')";
		// $result = dbQuery($sql);
		$checkSQL = "SELECT * FROM students_courses WHERE course_id = '$id' AND matric = '$matric'";
		$result1 = dbQuery($checkSQL);
		if (mysqli_num_rows($result1) > 0) {
			$_SESSION['msg'] = 'You have registered these courses already';
...
function addCourses_admin() {
	...
	for ($i=1; $i<=$loop; $i++) {
		...
		$checkSQL = "SELECT name FROM courses WHERE name = '$name'";
		$result1 = dbQuery($checkSQL);
		if (mysqli_num_rows($result1) > 0) {
			$_SESSION['msg'] = 'Courses have been added already';
