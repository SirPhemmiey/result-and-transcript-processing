...
<table width="1100" border="0"align="center" cellpadding="0" cellspacing="1" class="graybox">
 ...
 <tr> 
  ...
  <tr id="entryTableHeader"> 
         <td><h3>:: Print Result :::::<a href="<?php echo WEB_ROOT; ?>../">Home</a>&nbsp;&nbsp;<a href="view.php?mod=employee&view=printDetail">Bach</a>
...
<?php
$sql ="SELECT * FROM yearone1st WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
	 $gss101 =$row['gss101'];
				$phy101 =$row['phy101'];
...
$sql ="SELECT * FROM yearone2nd WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
	extract($row);
				  $phy192 =$row['phy192'];
...
$sql ="SELECT * FROM yeartwo1st WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
		extract($row);
		 $cis213 =$row['cis213'];
...
$sql ="SELECT * FROM yeartwo2nd WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
		extract($row);
		 		 $cis214 =$row['cis214'];
...
$sql ="SELECT * FROM yearthree1st WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
		extract($row);
		 		  $cis371 =$row['cis371'];
	...
	$sql ="SELECT * FROM yearthree2nd WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
		extract($row);
		 		 $cis372 =$row['cis372'];
	...
	$sql ="SELECT * FROM final1st WHERE std_id =$id";
	$result = dbQuery($sql);
	while($row = dbFetchAssoc($result)) {
		extract($row);
		 		$cis401 =$row['cis401'];
...
<p id="non-printable">Powered By: <a href="http://www.Springlight.org/">Springlight Technology</a></p>
